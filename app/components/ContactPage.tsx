...
const ContactPage = () => {
  ...
  const { selectedAddOns } = usePricingStore();
  const {
    formState,
    setFormValue,
    submitForm,
  ...
  useEffect(() => {
    if (selectedAddOns.length > 0) {
      const addonNames = selectedAddOns.map((a) => a.title).join(", ");
      const customMessage = `Hi! I'm interested in a project including the following add-ons: ${addonNames}.`;
      setFormValue("message", customMessage);
    }
  }, [selectedAddOns, setFormValue]);

  const inputClasses =
    "w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-sm md:text-base appearance-none";
  const labelClasses =
    "block text-xs md:text-sm font-medium text-slate-400 uppercase tracking-wider";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28">
      ...
      >
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 md:mb-6 tracking-tight text-white">
          Get in{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        ...
        >
          <form
            ...
            className="space-y-5 md:space-y-6 text-white"
          >
            {/* Name & Email Row */}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => setFormValue("timeline", e.target.value)}
                className={inputClasses}
              >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) => setFormValue("socialHandle", e.target.value)}
                className={inputClasses}
                placeholder="@yourname or link"
              />
            </div>
            <div className="space-y-2">
